...
printResult :: [(State, CandidateForest)] -> IO ()
...
collect :: TokenInterface token => ActionLogs token ->[(State, CandidateForest)]
collect logs =
    let initState = 0
...
onLogs :: TokenInterface token => State -- ^ 
  -> [ActionLog token] -- ^ 
  -> [(State, CandidateForest)] -- ^ state |-> { candidate1 |-> n1, ..., candidatek |-> nk }
